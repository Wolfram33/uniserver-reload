...
**The controller is the medallion:** the main window is the round 3D Reload medallion itself, with the start buttons, utilities and status LEDs sitting on the metal. There is no title bar: drag the window by any free spot of the coin, use the small `–` / `×` buttons (or Alt+F4) to minimize and close, and open the menus from the tab row (*General*, *Extra*, *Apache*, ...). All menu entries are the same as before, only their home moved from a menu bar to the tabs. Minimizing with the tray icon enabled parks the medallion in the notification area; since 1.3.6 the tray icon has a right-click menu (*Show* / *Exit*), so the controller can be quit from there, and *Extra > Edit Win hosts file* no longer hides the window when the hosts editor opens.
...
* [x] Vhost names explain themselves: any ending is allowed, but browsers send a bare `buchhaltung.wolf` to the search engine because they do not know the ending, which looks like a broken vhost. The Create Vhost dialog now shows a live hint under the server name: reserved endings (`.test`, `.localhost`, `.example`) open directly, an unknown ending says to type `http://buchhaltung.wolf/` in full, `.dev`/`.app` warn that browsers force HTTPS, `.local` that it belongs to mDNS. The same note is repeated after the vhost is created
